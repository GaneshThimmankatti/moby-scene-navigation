 ...
 */
 ...
 TrackerWithCloudNode::TrackerWithCloudNode() : rclcpp::Node("tracker_with_cloud_node")
 {
   ...
   sync_ = std::make_shared<message_filters::Synchronizer<ApproximateSyncPolicy>>(ApproximateSyncPolicy(10));
                                         ...
                                         const ultralytics_ros::msg::YoloResult::ConstSharedPtr& yolo_result_msg)
 {
   ...
   RCLCPP_INFO(this->get_logger(), "CameraInfo frame: %s", camera_info_msg->header.frame_id.c_str());
   RCLCPP_INFO(this->get_logger(), "PointCloud frame: %s, width: %d, height: %d",
               cloud_msg->header.frame_id.c_str(), cloud_msg->width, cloud_msg->height);
   RCLCPP_INFO(this->get_logger(), "YOLO detections count: %zu", yolo_result_msg->detections.detections.size());
   ...
   RCLCPP_INFO(this->get_logger(), "Downsampled cloud has %lu points", downsampled_cloud->size());
   ...
   RCLCPP_INFO(this->get_logger(), "Transformed cloud has %lu points", transformed_cloud->size());
                                         ...
                                         sensor_msgs::msg::PointCloud2& combine_detection_cloud_msg)
 {
   ...
   for (size_t i = 0; i < yolo_result_msg->detections.detections.size(); i++)
   {
     ...
     RCLCPP_INFO(this->get_logger(), "Detection %lu: raw detection cloud has %lu points",
                 i, detection_cloud_raw->points.size());

     if (!detection_cloud_raw->points.empty())
     {
       pcl::PointCloud<pcl::PointXYZ>::Ptr detection_cloud(new pcl::PointCloud<pcl::PointXYZ>());
       detection_cloud = cloud2TransformedCloud(detection_cloud_raw, cam_model_.tfFrame(), header.frame_id, header.stamp);
       ...
       if (!closest_detection_cloud->points.empty())
       {
         *combine_detection_cloud += *closest_detection_cloud;
         createBoundingBox(detection3d_array_msg, closest_detection_cloud, yolo_result_msg->detections.detections[i].results);
       }
       else
       {
         RCLCPP_WARN(this->get_logger(), "No clusters found for detection %zu", i);
       }
     }
   }
   ...
   RCLCPP_INFO(this->get_logger(), "Final combined detection cloud has %lu points", combine_detection_cloud->points.size());
 }
     ...
     const std::vector<vision_msgs::msg::ObjectHypothesisWithPose>& detections_results_msg)
 {
   if (cloud->empty())
   {
     RCLCPP_WARN(this->get_logger(), "Empty cloud passed to createBoundingBox; skipping detection.");
     return;
   }
